...
function renderLeaderboard() {
  if (leaderboardMode === 'last') renderLastSessionLeaderboard();
  else renderAllTimeLeaderboard();
}
    ...
    html += `
      <div class="leaderboard-row" style="grid-template-columns:48px 1fr 100px 80px 80px 80px;opacity:0.4">
        ...
        <div class="player-info">
          ...
          <div><div class="player-name">${p.name}</div><div class="player-tag">0 serate · 0 game</div></div>
        </div>
...
function renderLastSessionLeaderboard() {
  ...
  scores.forEach(sc => {
    if (!byPlayer[sc.player_name]) {
      byPlayer[sc.player_name] = { name: sc.player_name, emoji: sc.emoji, team: sc.team_name, total: 0, games: 0 };
    }
  ...
  scores.forEach(sc => { if (sc.team_name && teamTotals[sc.team_name] !== undefined) teamTotals[sc.team_name] += parseInt(sc.score)||0; });
  const maxTeamTotal = Object.values(teamTotals).length ? Math.max(...Object.values(teamTotals)) : 0;
  ...
  sorted.forEach((p, i) => {
    ...
    const isSolo    = !p.team;
    const isWin     = !isSolo && p.team === winningTeam;
    const teamColor = isSolo ? 'var(--neon3)' : isWin ? 'var(--neon)' : 'var(--neon2)';
    const teamLabel = isSolo ? 'Singolo' : (p.team || '—');
    const resultTag = isSolo
      ? `<span class="team-tag" style="background:rgba(0,245,255,0.1);color:var(--neon3);border:1px solid rgba(0,245,255,0.3);font-size:0.65rem">SINGOLO</span>`
    ...
    html += `
      <div class="leaderboard-row" style="animation-delay:${delay}s;grid-template-columns:48px 1fr 90px 90px 100px 90px">
        ...
        <div class="stat-cell col-partite" style="font-size:0.75rem;color:${teamColor}">${teamLabel}</div>
        <div class="stat-cell">${resultTag}</div>
      </div>`;
  });
  document.getElementById('leaderboard-body').innerHTML = html ||
...
async function loadSessions() {
  try {
    ...
    document.getElementById('sessions-body').innerHTML = html ||
      '<div style="padding:1.5rem;text-align:center;color:var(--text-muted);font-size:0.8rem">Nessuna sessione ancora 🎳</div>';
...
function renderLastSession(s) {
  ...
  scores.forEach(sc => {
    if (!sc.team_name || !byTeam[sc.team_name]) {
      ...
      if (!soloPlayers[pname]) soloPlayers[pname] = { name: pname, emoji: sc.emoji, total: 0 };
      soloPlayers[pname].total += parseInt(sc.score) || 0;
      return;
    }
  ...
  let soloHtml = '';
  const soloList = Object.values(soloPlayers);
  if (soloList.length) {
    soloHtml = `
      <div class="team-block" style="border-color:rgba(0,245,255,0.3)">
        <div class="team-header" style="background:rgba(0,245,255,0.05)">
          <span class="team-name-lbl" style="color:var(--neon3)">👤 Giocatori Singoli</span>
        </div>
        <div class="team-players">
          ${soloList.map(p => `
            <div class="team-player-row">
              <span>${p.emoji || '🎳'} ${p.name}</span>
              <span class="team-player-score">${p.total}</span>
            </div>`).join('')}
        </div>
      </div>`;
  }
...
async function loadHof() {
    ...
    document.getElementById('hof-card').innerHTML = `
      <div class="hof-row" style="padding:1rem 1.2rem">
        <div>
          ...
          <div style="font-family:'Black Han Sans',sans-serif;font-size:1.4rem;color:var(--gold);text-shadow:0 0 15px rgba(255,215,0,0.4)">${d.record_assoluto ?? '—'}</div>
          <div class="hof-sub">${d.record_holder ? d.record_holder.emoji + ' ' + d.record_holder.name + ' · ' + formatDate(d.record_holder.date) : '—'}</div>
        </div>
  ...
  } catch (e) { console.error('Errore HoF:', e); }
}
...
function buildGameRows() {
  ...
  ['A', 'B'].forEach(team => {
    ...
    for (let i = 0; i < count; i++) addPlayerRow(team, selected[i] || null, numGames);
  });
  calcTotals();
}
...
async function saveSession() {
  ...
  try {
    ...
    if (data.success) {
      ...
      loadStats(); loadLeaderboard(); loadSessions(); loadHof();
    } else {
  ...
  container.innerHTML = cachedPlayers.map(p => {
    ...
    return `
      <button onclick="toggleSuggestPlayer(${p.id}, this)" style="
        font-family:'Barlow Condensed',sans-serif;font-size:0.8rem;font-weight:600;letter-spacing:0.05em;
        ...
        padding:0.3rem 0.6rem;border-radius:20px;cursor:pointer;transition:all 0.15s;display:flex;align-items:center;gap:0.3rem
...
async function suggestTeams() {
  if (suggestSelected.size < 2) { showToast('Seleziona almeno 2 giocatori', 'error'); return; }
  const btn = document.getElementById('btnSuggest');
  ...
  } catch(e) { showToast('Errore nel calcolo', 'error'); }
  btn.disabled = false;
...
function renderSuggestResult(data) {
  ...
  function teamHtml(team, players, score, color, chem) {
    const chemHtml = chem.length
      ? chem.map(c => `<div style="font-size:0.68rem;font-family:'Share Tech Mono',monospace;color:var(--text-muted);margin-top:0.2rem">
          ${c.p1} + ${c.p2}: <span style="color:${c.pct >= 60 ? 'var(--neon)' : c.pct >= 40 ? 'var(--neon3)' : 'var(--neon4)'}">${c.pct}% win</span>
        </div>`).join('')
      : '';
    return `
      <div style="border:1px solid ${color}44;border-radius:6px;overflow:hidden;flex:1">
        ...
        <div style="padding:0.5rem 0.8rem">
          ${players.map(p => `<div style="font-size:0.85rem;font-weight:600;margin-bottom:0.2rem">
            ${p.emoji||'🎳'} ${p.name}
            <span style="font-size:0.65rem;color:var(--text-muted);font-family:'Share Tech Mono',monospace;font-weight:normal">
              ${p.media_storica > 0 ? p.media_storica : '—'} ${p.media_recente ? `· forma ${p.media_recente}` : ''}
            </span>
          </div>`).join('')}
          ${chemHtml}
        </div>
...
function saveFotoClassifica() {
  if (window.leaderboardMode === 'last') saveClassificaUltimaSerata();
  else saveClassifica();
}
