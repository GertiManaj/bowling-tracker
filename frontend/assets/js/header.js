...
(function () {
  ...
  function buildHTML(active, extraBtn) {
    ...
    const headerHtml = '<header><div class="header-glow"></div><div class="header-inner"><a href="index.html" class="logo" style="text-decoration:none"><div class="logo-pin">🎳</div><div class="logo-text"><span class="logo-title">STRIKE ZONE</span><span class="logo-sub">Bowling Tracker v1.0</span></div></a><nav>' + navHtml + '</nav><div class="header-actions"><button id="themeToggle" class="theme-toggle" onclick="toggleTheme()" title="Cambia tema">☀️</button><button class="btn-share" onclick="shareLink()">🔗 Condividi</button><button class="btn-login auth-hidden" id="btnLogin" onclick="openLoginModal()">🔐 Accedi</button><div class="hamburger-wrap"><button class="btn-hamburger" onclick="toggleHamburgerMenu()" title="Menu">☰</button><div class="hamburger-menu" id="hamburgerMenu"><div class="hamburger-admin-section auth-required"><div class="hamburger-label">Azioni Admin</div><button class="hamburger-item" onclick="openModal();closeHamburgerMenu()">🎳 Nuova Partita</button>' + (extraBtn === 'giocatori' ? '<button class="hamburger-item" onclick="openAddModal();closeHamburgerMenu()">➕ Nuovo Giocatore</button>' : '') + '<button class="hamburger-item" onclick="exportData();closeHamburgerMenu()">💾 Backup Database</button><div class="hamburger-divider"></div><button class="hamburger-item hamburger-logout" onclick="logout();closeHamburgerMenu()">🚪 Esci</button></div><div class="hamburger-guest-section auth-hidden"><div class="hamburger-label">Accesso</div><div class="hamburger-guest-msg">Non sei amministratore.<br>Accedi per gestire i dati.</div><button class="hamburger-item" onclick="openLoginModal();closeHamburgerMenu()">🔐 Accedi come Admin</button></div></div></div></div></div></header>';
